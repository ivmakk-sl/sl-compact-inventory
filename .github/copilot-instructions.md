...
- **A patch must not break the game.** Each patch body sits in a try/catch that logs the error, so item and save loading fall back to the game's own behavior.

## Structure and tests

- **Pure logic is separated and tested.** Logic that does not need the running game (the rule for which config entries change) lives in its own file with no BepInEx or Il2Cpp reference, unit-tested under `tests/`. Flag new pure logic in `Plugin.cs`, and new pure logic with no test.
- **Patches** prefer a postfix, and tie the `Harmony` instance to the plugin GUID.
- **One target, one effect.** The mod changes only `Config_Item.Size`. Flag a patch that writes any other item stat, a save field, or bag placement logic.
...
- **The plugin GUID never changes.** It is `com.ivmakk.survivallog.compactinventory`, the BepInEx identity and the config file name. Flag any edit to it.
